...
// 引入图片资源
import CAMREA from './img/camera.png';

// 配置的固定数据
// 熟人已有头像数
const myFriendCount = 28;
// 陌生人头像地址
const OthersSrc = 'http://lorempixel.com/';
...
class App extends React.Component {
  ...
  // 点赞中的朋友数量
  onChangeHeartsFriends = ev => {
    if (ev.currentTarget.value >= myFriendCount) {
      return;
    }
    // if ((Number(ev.currentTarget.value) && Number(ev.currentTarget.value) > 0 && Number(ev.currentTarget.value) < 11) || Number(ev.currentTarget.value) == '') {
    this.setState({
      heartsFriends: ev.currentTarget.value
    });
    // }
  }
  ...
  handleSelect = (time) => {
    let publishTime = '';
    publishTime = `${time.getFullYear()}-${time.getMonth() + 1}-${time.getDate()} ${time.getHours()}:${time.getMinutes()}`;
    this.setState({
      time,
      isOpen: false,
      ...
      return (
        <div className="root">
          ...
          <div className={`input-info`}>
            <label>朋友数量</label>
            <input
              onChange={this.onChangeHeartsFriends}
              type="number"
              placeholder={`最大值为${myFriendCount}，数量包含于总数中`}
              value={this.state.heartsFriends}
            />
          </div>
          <div className={`input-info`} onClick={this.handleClick}>
            <label>发布时间</label>
            <input
              placeholder="选择时间"
    ...
    return (
      <div>
        <WeChat
          ...
          heartsFriends={this.state.heartsFriends}
          publishTimeObj={this.state.time}
        />
      </div>
    );
  }
...
class WeChat extends React.Component {
          ...
          <div className="item" key={index}>
            ...
            <div className="detail">
              <div className="item-info">
                ...
                <span className="time">
                  {this.getShowTimeStr(this.props.publishTimeObj)}
                </span>
              </div>
  ...
  renderHearts() {
    ...
    if (this.props.heartsCount > 0) {
      let allImgs = new Array(Number(this.props.heartsCount));
      allImgs = allImgs.fill(1);
      // 好友占位
      allImgs = allImgs.map((i, index) => {
        if (index < this.props.heartsFriends) {
          return index;
        }else{
          return '';
        }
      ...
      allImgs = allImgs.map((item, index) => {
        if(item != ''){
          return `./img/head/${item + 1}.jpg`
        }else{
          return `${OthersSrc}/${76+index*2}/${76+index*2}`;
        }
      })
      for (let i = 0; i < this.props.heartsCount; i++) {
        hearts.push(
          <img src={`${allImgs[i]}`} className="head" alt="" />
        );
      }
      return (
  ...
  // 发布时间渲染
  renderPublishTime() {
    return this.getShowTimeStr(this.props.publishTimeObj, true);
  }
  ...
  renderUserImg() {
    if (this.props.userImg.length == 0) {
      // 用户没有上传头像，使用默认头像
      return (<img className="head" src="./img/default.jpg" alt="" />);
    } else {
      //  用户上传头像
      return (<img className="head" src={`data:image/${this.props.userImg[0].type};base64,${this.props.userImg[0].base64code}`} alt="" />);
    }
  }
...
class UploadImage extends React.Component {
  ...
  constructor(props) {
    super(props);
    let imgData = [];
    if (!!this.props.originImgData && this.props.originImgData.length > 0) {
